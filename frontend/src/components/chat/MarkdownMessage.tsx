  ...
  return (
    <div className="relative group my-3">
      <div className="flex items-center justify-between px-3 py-1.5 bg-[#0d1117] border border-border rounded-t-lg border-b-0">
        ...
        <button onClick={copy} className="flex items-center gap-1 text-[10px] text-muted-foreground hover:text-foreground transition-colors opacity-0 group-hover:opacity-100">
      ...
      <ReactMarkdown
        ...
        components={{
          code({ node, inline, className, children, ...props }: { node?: unknown; inline?: boolean; className?: string; children?: React.ReactNode; [key: string]: unknown }) {
            ...
            return <code className="px-1.5 py-0.5 bg-muted border border-border rounded text-cyan-400 text-[0.8em] font-mono" {...props}>{children}</code>;
          },
          table: ({ children }) => (
